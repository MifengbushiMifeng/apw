...
class Response(object):
    ...
    def set_cookie(self, name, value, max_age=None, expires=None,
        ...
        :param max_age: optional, seconds of cookies's max age.
        :param expires: optional, unix timestamp, datetime or date object that
                indicate on absolute time of the expiration time of cookie.
                Note that if expires specified, the max_age will be ignored.
        :param path: the cookie path, default is '/'
        :param domain: the cookie domain, default is None
        :param secure: if the cookie is secure, default is False
        :param http_only: if the cookie is http only, default is False
        :return: the cookie
        """
        if not hasattr(self, '_cookies'):
            self._cookies = {}
        ...
        if expires is not None:
            if isinstance(expires, (float, int, long)):
                L.append('Expires=%s' % datetime.datetime.
                         fromtimestamp(expires, UTC_0).strftime('%a, %d-%b-%Y %H:%M:%S GMT'))
...
class WSGIApplication(object):
    ...
    def _check_not_running(self):
        if self._running:
            raise RuntimeError('Cannot modify WSGIApplication when it running')
    ...
    def add_module(self, mod):
        ...
        for name in dir(m):
            fn = getattr(m, name)
            if callable(fn) and hasattr(fn, '__web__route') and hasattr(fn, '__web_method__'):
                self.add_url(fn)
